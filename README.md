...
Signatures are optional metadata for `analyzeFormulaTypes`. They describe required `parameters`, optional trailing `optionalParameters`, assumed argument conversions, and a result category; they do not change or validate the runtime implementation. A custom function must honor its declared signature. Pass `{ registry }` as the third analysis argument to apply registered signatures, overrides, and removals. Passing the registry directly remains supported. The analyzer includes signatures for math, text, and `IS*` inspection functions, plus category-dependent analysis for `N`, `T`, `VALUE`, `ROWS`, and `COLUMNS`; other built-ins without specific analysis remain `Unknown` until audited.
